...
class SalesInvoiceController extends Controller
{
    ...
    public function store(StoreSalesInvoiceRequest $request)
    {
        if(Auth::user()->can('create-sales-invoices')){
            ...
            // Fire hook for country packages and industry addons
            \App\Classes\Hooks::do_action('invoice_created', $invoice);
    ...
    private function calculateTotals($items)
    {
        ...
        $taxResult = \App\Classes\Hooks::apply_filters('calculate_tax', [
            'subtotal' => $subtotal,
            'total_tax' => 0,
        ...
        return [
            'subtotal' => $subtotal,
            'tax_amount' => $taxResult['total_tax'] ?? 0,
            'discount_amount' => $totalDiscount,
            'total_amount' => ($taxResult['subtotal'] ?? $subtotal) + ($taxResult['total_tax'] ?? 0) - $totalDiscount
        ];
    }
