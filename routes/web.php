...
use App\Http\Controllers\CountryPackageController;
...
// Country-specific landing page routes
// Country codes are dynamically loaded from installed Country packages
$countryCodes = implode('|', array_keys(app(\App\Services\CountryDetectionService::class)->getSupportedCountries() ?? ['gb' => 'GB', 'us' => 'US', 'ae' => 'AE']));

Route::get('/{country?}', [\Workdo\LandingPage\Http\Controllers\LandingPageController::class, 'index'])
    ->where('country', $countryCodes)
    ->name('landing.country');

// Country-specific landing page (blade for SEO)
Route::get('/{country}/seo', [\Workdo\LandingPage\Http\Controllers\LandingPageController::class, 'seo'])
    ->where('country', $countryCodes)
    ->name('landing.country.seo');

// UK Country Package Routes (loaded by CountryGBServiceProvider)
// Route::prefix('uk')->name('uk.')->middleware(['web', 'auth'])->group(function () {
...
// Country Package Management (Superadmin)
Route::middleware(['web', 'auth'])->prefix('country-packages')->name('country-packages.')->group(function () {
    Route::get('/', [App\Http\Controllers\CountryPackageController::class, 'index'])->name('index');
    Route::post('/{packageName}/toggle', [App\Http\Controllers\CountryPackageController::class, 'toggle'])->name('toggle');
    Route::get('/{countryCode}/stats', [App\Http\Controllers\CountryPackageController::class, 'stats'])->name('stats');
});
