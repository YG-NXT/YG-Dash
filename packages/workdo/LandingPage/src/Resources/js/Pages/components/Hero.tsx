...
export default function Hero({ settings, countryHero }: HeroProps) {
    ...
    const title = countryHero?.title || sectionData.title || 'Transform Your Business with WorkDo Dash';
    const subtitle = countryHero?.subtitle || sectionData.subtitle || 'The complete all-in-one business management solution that combines Project Management, Accounting, HRM, CRM, POS, and Product Management into a single powerful platform. Streamline operations, boost productivity, and grow your business with our integrated suite of tools.';
    const primaryButtonText = countryHero?.cta_text || sectionData.primary_button_text || 'Start Free Trial';
    const primaryButtonLink = countryHero?.cta_link || sectionData.primary_button_link || route('register');
