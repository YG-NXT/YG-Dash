...
export default function Pricing(props: PricingProps) {
    ...
    const countryPricing = settings?.pricing;
    const displayPlans = countryPricing?.plans?.length > 0 ? countryPricing.plans : plans;
    const currency = countryPricing?.currency || 'USD';
    const symbol = countryPricing?.symbol || '$';
    const annualDiscount = countryPricing?.annual_discount;

    // Find the plan with the highest order count for "Most Popular" badge
    const mostPopularPlanId = displayPlans.length > 0 
        ? displayPlans.reduce((prev: any, current: any) => 
            (current.orders_count || 0) > (prev.orders_count || 0) ? current : prev
          ).id
        : null;

    const formatCountryPrice = (amount: number): string => {
        return `${symbol}${amount.toLocaleString()}`;
    };

    return (
        <>
            <Head title={`Pricing - ${currency}`} >
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center mb-12">
                        ...
                        <p className="text-xl text-gray-600 max-w-3xl mx-auto mb-8">
                            {pricingSettings.subtitle || t('Choose the perfect plan for your business needs')}
                        </p>
                                                    ...
                                                    <div>
                                                        <div className="flex items-baseline justify-center gap-1 mb-2">
                                                            <span className="text-5xl font-black text-gray-900">
                                                                {priceType === 'monthly' ? formatCountryPrice(plan.package_price_monthly) : formatCountryPrice(plan.package_price_yearly)}
                                                            </span>
